...
if pagina == "Resumen":
    ...
    with tab2:
        ...
        responsable = c3.text_input("Responsable", value="Yani", key="nuevo_responsable")
    ...
    with tab1:
        if prospectos.empty:
            ...
            with st.expander("Editar ficha", expanded=True):
                ...
                responsable = c3.text_input("Responsable", value=r["responsable"] or "", key=f"edit_responsable_{pid}")
                    ...
                    if giro_base == "Otro" and not giro_otro.strip():
                        ...
                        execute("""UPDATE prospectos SET negocio=?, contacto=?, telefono=?, correo=?, giro=?, origen=?, etapa=?,
                            ...
                            (negocio.strip(), contacto.strip(), telefono.strip(), correo.strip(), giro, origen, etapa, prioridad,
                             ", ".join(locales), superficie, precio_m2, renta, probabilidad, proximo.isoformat(), responsable.strip(),
