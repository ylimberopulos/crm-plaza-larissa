...
def eliminar_cliente(cliente_id):
    cliente_df = query_df("SELECT prospecto_id, locales_interes FROM clientes WHERE id=?", (cliente_id,))
    if cliente_df.empty:
        return

    prospecto_id = cliente_df.iloc[0]["prospecto_id"]
    locales = [x.strip() for x in str(cliente_df.iloc[0]["locales_interes"] or "").split(",") if x.strip()]
    documentos = query_df("SELECT ruta_archivo FROM documentos_cliente WHERE cliente_id=?", (cliente_id,))
